...
def broker_rows(dates, accumulating=True):
    rows=[]
    for d in dates:
        for code, scale in [("YP",1.0),("BK",.7),("AK",.5)]:
            buy=10_000_000*scale if accumulating else 3_000_000*scale; sell=3_000_000*scale if accumulating else 10_000_000*scale
            rows.append({"ticker":"TEST","trade_date":d,"broker_code":code,"buy_value":buy,"sell_value":sell,"buy_volume":100_000*scale,"sell_volume":30_000*scale,"buy_avg":104.0,"sell_avg":105.0})
...
def test_direct_accumulation_scores_above_no_data():
    ...
    assert feat["coverage_pct"]==100.0; assert feat["accumulation_score"]>70; assert feat["estimated_smart_money_cost"] is not None
...
def test_prior_accumulators_reversing_to_sell_triggers_distribution_warning():
    ...
    for i,d in enumerate(dates):
        reversal=i>=55
        for code,scale in [("YP",1.0),("BK",0.8),("AK",0.6)]:
