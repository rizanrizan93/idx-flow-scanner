...
class IdxOfficialAccessBlocked(RuntimeError):
    ...
    This is intentionally distinct from a normal no-trading-day/empty response.
    Repeating the same Cloudflare 401/403 request for dozens of historical dates
    only wastes runtime and cannot improve evidence coverage during that run.
    """
...
def fetch_idx_stock_summary(trade_date: date | str, *, retries: int = 3, timeout: float = 25.0) -> pd.DataFrame:
    ...
    A 401/403 from the IDX edge is treated as a run-level access block instead of
    a date-specific miss. The caller can then stop historical probing immediately.
    A 429 remains retryable because throttling can clear within the same run.
    """
        ...
        try:
            ...
            if response.status_code in {401, 403}:
                raise IdxOfficialAccessBlocked(
                    f"IDX official stock-summary access blocked with HTTP {response.status_code}"
                )
...
def fetch_idx_official_flow_history(
    ...
    *,
    ...
    If IDX blocks this cloud environment, stop immediately. Empty evidence remains
    empty and therefore neutral/guarded; it is never synthesized from OHLCV.
    """
