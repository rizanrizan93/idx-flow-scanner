...
def install_universe_wide_idx_broker(streamlit_app: Any) -> None:
    """Add verified broker/participant evidence only during finalist verification.

    Production order is deliberately PRICE_PROXY + foreign evidence -> guarded
    Top 5 -> broker verification. Direct broker evidence must never enter the
    first-pass ranking because the guarded cohort is explicitly PRICE_PROXY.
    """
    ...
    def finalist_loader_with_idx(
        ...
        *,
        ...
        if official.empty:
            ...
            stats["broker_enrichment_scope"] = "FINALISTS_ONLY"
            return indexalpha_frame, stats
        combined = (
            pd.concat([official, indexalpha_frame], ignore_index=True)
            if indexalpha_frame is not None and not indexalpha_frame.empty
            else official
        )
        try:
            ...
            stats["universe_idx_broker_selector_error"] = f"{type(exc).__name__}: {str(exc)[:160]}"
