...
create function flow_private.backfill_yahoo_ohlcv(
  p_tickers text[],
...
begin
  foreach v_ticker in array p_tickers loop
    ...
    begin
      ...
      select h.status, h.content into v_http_status, v_content
      ...
      if v_http_status <> 200 then
        ...
        where id = v_audit_id;
        out_ticker := v_ticker; out_http_status := v_http_status; out_rows_accepted := 0; out_status := 'HTTP_ERROR';
        return next;
        continue;
      end if;
      ...
      if v_result is null or jsonb_typeof(v_result->'timestamp') <> 'array' then
        ...
        where id = v_audit_id;
        out_ticker := v_ticker; out_http_status := v_http_status; out_rows_accepted := 0; out_status := 'NO_DATA';
        return next;
        continue;
      end if;
      ...
      on conflict on constraint flow_daily_prices_pkey do update
      ...
      set completed_at = now(),
          ...
          freshness_date = (select max(p.trade_date) from public.flow_daily_prices p where p.ticker = v_ticker),
          details = details || jsonb_build_object('http_status', v_http_status, 'source', 'YAHOO_CHART_DB')
      where id = v_audit_id;

      out_ticker := v_ticker;
      out_http_status := v_http_status;
      out_rows_accepted := v_rows;
      out_status := case when v_rows >= 80 then 'COMPLETED' else 'INSUFFICIENT' end;
      return next;
    exception when others then
      ...
      where id = v_audit_id;
      out_ticker := v_ticker; out_http_status := coalesce(v_http_status, 0); out_rows_accepted := 0; out_status := 'FAILED';
      return next;
    end;
  end loop;
end;
