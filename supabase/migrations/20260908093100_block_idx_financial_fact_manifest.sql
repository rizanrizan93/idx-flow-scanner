...
begin
  ...
  if v_meta->>'schema_version' is distinct from 'BLOCK_IDX_FINANCIAL_FACT_ARTIFACT_META_V5_3'
     or coalesce((v_meta->>'source_run_id')::bigint,-1) <> v_run_id
     or v_source_head_sha !~ '^[0-9a-f]{40}$'
     or v_meta->>'manifest_sha256' is distinct from v_actual_sha
  ...
  if (select count(*) from public.flow_financial_metric_catalog_v5 where catalog_sha256 = v_expected_catalog_hash) <> 19
     or v_payload->>'schema_version' is distinct from 'BLOCK_IDX_FINANCIAL_FACT_MANIFEST_V5_3'
     ...
     or coalesce((v_payload->>'failed_filing_rows')::integer,-1) <> coalesce((v_payload->>'permanent_excluded_filing_rows')::integer,-2)
     or coalesce((v_payload->>'permanent_excluded_filing_rows')::integer,-1) < 0
     or coalesce((v_payload->>'retryable_failure_rows')::integer,-1) <> 0
     or coalesce((v_payload->>'parser_or_contract_failure_rows')::integer,-1) <> 0
     or v_payload->>'source_head_sha' is distinct from v_source_head_sha
     or coalesce((v_payload->>'source_run_id')::bigint,-1) <> v_run_id
     or coalesce((v_payload->>'selected_filing_rows')::integer,-1) <> coalesce((v_payload->>'parsed_filing_rows')::integer,-2) + coalesce((v_payload->>'permanent_excluded_filing_rows')::integer,-3)
     ...
     or jsonb_typeof(v_payload->'shards') is distinct from 'array'
  ...
  if v_payload->'exclusions'->>'file_name' is distinct from 'exclusions.json'
     or coalesce(v_payload->'exclusions'->>'sha256','') !~ '^[0-9a-f]{64}$'
     or coalesce((v_payload->'exclusions'->>'filing_rows')::integer,-1) <> (v_payload->>'permanent_excluded_filing_rows')::integer then
    raise exception 'FINANCIAL_FACT_EXCLUSION_MANIFEST_CONTRACT';
  end if;
  select status, content into v_status, v_exclusion_content
  from extensions.http_get(regexp_replace(p_manifest_url, '/manifest\.json$', '/exclusions.json')::varchar);
  if v_status is distinct from 200 or v_exclusion_content is null
     or octet_length(convert_to(v_exclusion_content,'UTF8')) is distinct from (v_payload->'exclusions'->>'bytes')::integer
     or encode(extensions.digest(convert_to(v_exclusion_content,'UTF8'),'sha256'),'hex') is distinct from v_payload->'exclusions'->>'sha256' then
    raise exception 'FINANCIAL_FACT_EXCLUSION_HASH_OR_BYTES_MISMATCH';
  end if;
  v_exclusions := v_exclusion_content::jsonb;
  if v_exclusions->>'schema_version' is distinct from 'BLOCK_IDX_FINANCIAL_FACT_EXCLUSIONS_V5_3'
     or v_exclusions->>'source_head_sha' is distinct from v_source_head_sha
     or (v_exclusions->>'source_run_id')::bigint is distinct from v_run_id
     or (v_exclusions->>'production_scoring_changed')::boolean is distinct from false
     or jsonb_typeof(v_exclusions->'rows') is distinct from 'array'
     or jsonb_array_length(v_exclusions->'rows') is distinct from (v_payload->>'permanent_excluded_filing_rows')::integer
     or (v_exclusions->>'permanent_excluded_filing_rows')::integer is distinct from (v_payload->>'permanent_excluded_filing_rows')::integer
     or v_exclusions->'exclusion_class_counts' is distinct from v_payload->'exclusion_class_counts' then
    raise exception 'FINANCIAL_FACT_EXCLUSION_LEDGER_CONTRACT';
  end if;
  select count(*) into v_bad
  from jsonb_array_elements(v_exclusions->'rows') x
  left join public.flow_financial_filing_evidence_v5 p on p.filing_id=x.value->>'filing_id'
  where p.filing_id is null or p.ticker is distinct from x.value->>'ticker'
     or p.file_url is distinct from x.value->>'file_url'
     or p.published_at is distinct from (x.value->>'published_at')::timestamptz
     or p.source_verified is not true or p.publication_time_verified is not true or p.point_in_time_eligible is not true
     or coalesce(x.value->>'failure_class','') not in ('OFFICIAL_ATTACHMENT_UNAVAILABLE_CONFIRMED','OFFICIAL_ATTACHMENT_CORRUPT_CONFIRMED')
     or (x.value->>'retryable')::boolean is distinct from false
     or (x.value->>'point_in_time_identity_preserved')::boolean is distinct from true
     or coalesce(x.value->>'classification_reason','') = ''
     or coalesce(x.value->>'exact_profile_resolution_state','') = ''
     or jsonb_typeof(x.value->'official_mirror_attempts') is distinct from 'array'
     or exists(select 1 from public.flow_financial_fact_evidence_v5 f where f.filing_id=p.filing_id)
     or exists(select 1 from public.flow_financial_fact_manifest_filing_v5 f where f.filing_id=p.filing_id and f.manifest_sha256=v_actual_sha);
  if v_bad <> 0
     or (select count(distinct x.value->>'filing_id') from jsonb_array_elements(v_exclusions->'rows') x) <> jsonb_array_length(v_exclusions->'rows')
     or (select coalesce(jsonb_object_agg(failure_class,n),'{}'::jsonb) from (
          select x.value->>'failure_class' failure_class,count(*) n from jsonb_array_elements(v_exclusions->'rows') x group by 1
        ) c) is distinct from v_payload->'exclusion_class_counts' then
    raise exception 'FINANCIAL_FACT_EXCLUSION_IDENTITY_CONFLICT:%',v_bad;
  end if;
  ...
  insert into public.flow_financial_fact_manifest_v5(
    ...
    metric_catalog_sha256, selected_filing_rows, parsed_filing_rows, permanent_excluded_filing_rows, manifest_raw, exclusions_raw, fact_rows,
    ...
    (v_payload->>'permanent_excluded_filing_rows')::integer,
    v_content, v_exclusion_content,
  ...
  insert into public.flow_financial_fact_manifest_exclusion_v5(
    manifest_sha256,filing_id,ticker,failure_class,retryable,point_in_time_identity_preserved,classification_reason,provenance_payload
  ) select v_actual_sha,x.value->>'filing_id',x.value->>'ticker',x.value->>'failure_class',false,true,x.value->>'classification_reason',x.value
    from jsonb_array_elements(v_exclusions->'rows') x
    on conflict(manifest_sha256,filing_id) do nothing;
  if exists (
    select 1 from public.flow_financial_fact_manifest_exclusion_v5 e
    join jsonb_array_elements(v_exclusions->'rows') x on x.value->>'filing_id'=e.filing_id
    where e.manifest_sha256=v_actual_sha and e.provenance_payload is distinct from x.value
  ) then raise exception 'FINANCIAL_FACT_EXCLUSION_PROVENANCE_CONFLICT'; end if;
  ...
  return jsonb_build_object(
    ...
    'selected_filing_rows',(v_payload->>'selected_filing_rows')::integer,
    'permanent_excluded_filing_rows',(v_payload->>'permanent_excluded_filing_rows')::integer,
