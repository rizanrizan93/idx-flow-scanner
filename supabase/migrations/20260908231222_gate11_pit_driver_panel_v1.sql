-- Gate 11: PIT weekly ticker x signal_date driver panel.
-- Reuses canonical market-memory and financial-v5 sources; no source duplication.

create table if not exists public.flow_driver_signal_panel_v1 (
  ...
  market_regime_value numeric,
  source_identity text not null,
  revision_identity text not null,
  evidence_timestamp timestamptz not null,
  effective_availability_date date not null,
  target_date_5d date,target_date_20d date,target_date_60d date,
  forward_return_5d_pct numeric,forward_return_20d_pct numeric,forward_return_60d_pct numeric,
  alpha_vs_ihsg_5d_pct numeric,alpha_vs_ihsg_20d_pct numeric,alpha_vs_ihsg_60d_pct numeric,
  alpha_vs_sector_5d_pct numeric,alpha_vs_sector_20d_pct numeric,alpha_vs_sector_60d_pct numeric,
  mfe_5d_pct numeric,mfe_20d_pct numeric,mfe_60d_pct numeric,
  mae_5d_pct numeric,mae_20d_pct numeric,mae_60d_pct numeric,
  sector_target_state text not null,
...
create table if not exists public.flow_driver_observation_panel_v1 (
  panel_contract text not null,
  signal_date date not null,
  ticker text not null,
  driver_id text not null,
  driver_state text not null check(driver_state in ('AVAILABLE','MISSING','STALE','INVALID','NOT_APPLICABLE','INSUFFICIENT_HISTORY')),
  raw_value numeric,
  transformed_value numeric,
  normalized_value numeric,
  bottom_signal boolean not null default false,
  top_signal boolean not null default false,
  evidence_timestamp timestamptz,
  effective_availability_date date,
  source_identity text not null,
  revision_identity text not null,
  stale_status text not null,
  missingness_status text not null,
  provenance jsonb not null,
  built_at timestamptz not null,
  production_influence_enabled boolean not null default false check(production_influence_enabled=false),
  primary key(panel_contract,signal_date,ticker,driver_id),
  foreign key(panel_contract,signal_date,ticker)
    references public.flow_driver_signal_panel_v1(panel_contract,signal_date,ticker)
    on delete cascade
);

create index if not exists flow_driver_observation_v1_driver_date_idx
  on public.flow_driver_observation_panel_v1(driver_id,signal_date)
  where driver_state='AVAILABLE';
create index if not exists flow_driver_observation_v1_ticker_date_idx
  on public.flow_driver_observation_panel_v1(ticker,signal_date);

create table if not exists public.flow_driver_coverage_v1 (
  panel_contract text not null,
  entity_type text not null check(entity_type in ('DRIVER','FAMILY')),
  entity_id text not null,
  total_rows bigint not null,
  available_rows bigint not null,
  missing_rows bigint not null,
  stale_rows bigint not null,
  invalid_rows bigint not null,
  not_applicable_rows bigint not null,
  insufficient_history_rows bigint not null,
  coverage_pct numeric not null,
  stale_pct numeric not null,
  invalid_pct numeric not null,
  calculated_at timestamptz not null,
  primary key(panel_contract,entity_type,entity_id)
);

create table if not exists public.flow_driver_panel_manifest_v1 (
  panel_contract text primary key,
  registry_version text not null,
  signal_rows bigint not null,
  observation_rows bigint not null,
  signal_dates integer not null,
  tickers integer not null,
  sectors integer not null,
  available_observations bigint not null,
  coverage_pct numeric not null,
  missing_pct numeric not null,
  stale_pct numeric not null,
  invalid_pct numeric not null,
  leakage_audit jsonb not null,
  leakage_count bigint not null check(leakage_count=0),
  revision_leakage_count bigint not null check(revision_leakage_count=0),
  target_leakage_count bigint not null check(target_leakage_count=0),
  build_state text not null check(build_state in ('COMPLETE','FAILED')),
  built_at timestamptz not null,
  production_influence_enabled boolean not null default false check(production_influence_enabled=false)
);

alter table public.flow_driver_signal_panel_v1 enable row level security;
alter table public.flow_driver_observation_panel_v1 enable row level security;
alter table public.flow_driver_coverage_v1 enable row level security;
alter table public.flow_driver_panel_manifest_v1 enable row level security;
revoke all on public.flow_driver_signal_panel_v1,public.flow_driver_observation_panel_v1,
 public.flow_driver_coverage_v1,public.flow_driver_panel_manifest_v1 from public,anon,authenticated,service_role;
grant select,insert,update,delete on public.flow_driver_signal_panel_v1,public.flow_driver_observation_panel_v1,
 public.flow_driver_coverage_v1,public.flow_driver_panel_manifest_v1 to service_role;
...
begin
  ...
  with market_daily as (
    ...
    left join breadth b using(as_of_date)
