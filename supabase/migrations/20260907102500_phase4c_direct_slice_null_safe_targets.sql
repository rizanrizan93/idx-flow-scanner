-- Phase 4C direct-slice runtime hotfix.
-- Some horizons intentionally have no winner/loser/multibagger boolean target.
-- Build those dynamic SQL expressions explicitly instead of formatting NULL identifiers.
...
begin
  ...
  v_loser_expr:=case when v_loser is null then 'null::boolean' else format('c.%I',v_loser) end;
  v_multi_expr:=case when v_multi is null then 'null::boolean' else format('c.%I',v_multi) end;
  ...
  $q$,
    ...
    v_winner_expr,v_loser_expr,v_multi_expr,
